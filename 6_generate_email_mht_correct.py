#!/usr/bin/env python3
# Generate Chinese email in MHT format with correct MIME structure and CRLF line endings
...
# Generate HTML body exactly matching template structure
...
# Create MHT file with proper CRLF line endings and structure
def create_mht_correct(html_content, output_path):
    boundary = "----=_NextPart_01DC1DB8.F194A1B0"
    
    # Encode HTML to quoted-printable with GB2312
    try:
    ...
    mht_lines = [
        ...
        "",
        f"--{boundary}",
        "Content-Location: file:///C:/6ECBBD04/Untitled.htm",
    ...
    with open(output_path, 'wb') as f:
        f.write(mht_content.encode('ascii'))

# Generate MHT
output_file = f'data/7_emails/{test_date}_email_correct.mht'
create_mht_correct(html_body, output_file)

print(f"✅ Correct MHT Email generated: {output_file}")
