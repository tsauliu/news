#!/usr/bin/env python3
# Generate Chinese email in MHT format with improved formatting and fonts
...
# Generate MHT
output_file = f'data/7_emails/{test_date}_email_improved.mht'
create_mht_improved(html_body, output_file)

print(f"✅ Improved MHT Email generated: {output_file}")
...
print(f"   - Podcast summaries: {len(podcast_summaries)} with prefixed takeaways")
print(f"\n📝 Improvements:")
print(f"   - Font: Chinese text uses 等线, English uses Calibri")
print(f"   - Title: 'Key News takeaway for Week – {test_date}'")
print(f"   - Sellside: 'Sellside highlights for Week – {test_date}' (bold)")
print(f"   - Sellside format: Date,Firm: Title (URL) with sub-bullets")
print(f"   - Podcast bullets: Prefixed with podcast name")